...
import { fromNotionDividerBlock } from "./from-notion-divider-block"
...
const block = {
  ...
  type: "divider",
  divider: {},
} as const satisfies DividerBlockObjectResponse

test("dividerブロックをマークダウンに変換できる", () => {
  const result = fromNotionDividerBlock(block)
  expect(result).toBe("---")
})
